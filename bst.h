...
typename BinarySearchTree<Key, Value>::iterator &BinarySearchTree<Key, Value>::iterator::operator++()
{
    ...
    //  BinarySearchTree<Key, Value>::iterator next(successor(this->current_));
    return *this;
}
...
void BinarySearchTree<Key, Value>::remove(const Key &key)
{
    ...
    if (del->getLeft() != NULL && del->getRight() != NULL)
    {
      ...
      nodeSwap(del, pred);
      if(del->getLeft() == NULL && del->getRight() == NULL){
       if(del->getParent()->getLeft() != NULL && del == del->getParent()->getLeft()){
    ...
    }else if (del->getRight() != NULL)
    {
      if(del != root_){
        ...
        del->getRight()->setParent(NULL);
        root_ = del->getRight();
      }
  
    }

    if(del == root_ && (del->getLeft() == NULL && del->getRight() == NULL)){
      isLoneRoot = 1;
    }
...
int BinarySearchTree<Key, Value>::height(Node<Key, Value> *curr) const
{
  ...
  int leftHeight = height(root_->getLeft()); 
  int rightHeight = height(root_->getLeft());
